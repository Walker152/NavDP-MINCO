...
def main(argv=None):
    ...
    if args.command == "run-suite": result = run_suite(args.config, args.backend, args.resume, retry_failed=args.retry_failed, dry_run=args.dry_run, analysis_only=args.analysis_only); print(json.dumps(result.__dict__))
