...
from experiments.visualizers.common import save_mock_figure
...
def analyze_suite(suite_dir):
    ...
    generate_result_tables(suite_dir, len(episodes), len(plans)); select_representative_cases(suite_dir, episodes); failures = generate_failure_report(suite_dir, episodes)
    plot_rows = []
    for index, name in enumerate(EXPERIMENT_NAMES):
        ...
        (directory / "summary_metrics.csv").write_text("metric,n,value,data_source\nepisode_count,%d,%d,SIMULATED\n" % (len(episodes), len(episodes)), encoding="utf-8")
        (directory / "paired_metrics.csv").write_text("metric,paired_n,delta,data_source\nmock_pair,0,,SIMULATED\n", encoding="utf-8")
        for filename in PLOTS.get(index, ()):
            path = save_mock_figure(directory / "plots" / filename, f"{exp_id}: {filename} (n={len(plans)})", values=[0.2, .5, .8])
            plot_rows.append({"experiment_id":exp_id, "plot_path":str(path.relative_to(suite_dir)) if path else "", "status":"generated" if path else "skipped", "skip_reason":"" if path else "no finite data", "data_source":"SIMULATED"})
        (directory / "report.md").write_text(f"# {exp_id}\n\n> **SIMULATED DATA — pipeline validation only.**\n\nEpisodes: {len(episodes)}; valid plans: {len(plans)}; planning cycles: {len(cycles)}.\n\nNo algorithm superiority is inferred.\n", encoding="utf-8")
    ...
    text = "# NavDP–MINCO Suite Report\n\n> **SIMULATED DATA — 工具链验收数据，不代表算法科研结论。**\n\n"
    text += f"## Configuration and provenance\n\nData source: SIMULATED; run files: {len(list(suite_dir.glob('experiments/*/*/*/*/*/run_config.json')))}.\n\n"
    ...
    text += "## Improvements, regressions and failures\n\nSynthetic cases exercise repaired, rejected, collision, timeout, HOT accepted and HOT rejected paths. No performance claim is made.\n\n"
    text += "## Missing fields and conclusion boundary\n\nFields unavailable to a synthetic backend remain empty and are not converted to zero. Real GPU/Isaac/NavDP/MINCO/MPC behavior remains unverified.\n\n"
