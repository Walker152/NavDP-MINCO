...
def generate_failure_report(suite_dir, episodes):
    ...
    with (reports / "failure_case_index.csv").open("w", newline="", encoding="utf-8") as stream:
        ...
        for row in failures: writer.writerow({"episode_uid":row.get("episode_uid", ""), "primary_reason":row.get("done_reason", "UNKNOWN"), "scene_label":row.get("scene_label", ""), "variant":row.get("variant", ""), "data_source":"SIMULATED"})
    text = "# Failure Case Report\n\n> SIMULATED DATA\n\n" + ("\n".join(f"- {row.get('episode_uid')}: {row.get('done_reason')}" for row in failures) or "No failures recorded.") + "\n"
