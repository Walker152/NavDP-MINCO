...
from experiments.designers.suite import expand_runs, load_suite, resolve_suite_behavior
...
def _run_config(run, scene, episodes, backend_name, data_source, manifest_id, video_required=None, trace_required=None, parameter_overrides=None):
    parameter_receipt = resolve_parameter_receipt(
        video_enabled=(
            video_required if video_required is not None else backend_name == "isaac"
        ),
        overrides=parameter_overrides,
    )
    parameters = parameter_receipt["effective"]
    ...
    for run_index, (run, expected_episode_count) in enumerate(zip(expanded_runs, episode_counts), start=1):
        ...
        try:
            ...
            if lifecycle.status == "ANALYZING":
                if behavior.analysis_enabled:
                    analyze_run(run_dir)
                lifecycle.transition(
                    "COMPLETE",
                    validation="PASS",
                    analysis="COMPLETE" if behavior.analysis_enabled else "SKIPPED_CONFIG",
                )
            progress.finish_run()
            completed += 1
