...
def load_manifest(path: Path | str) -> Manifest:
    ...
    for raw_scene in sorted(data.get("scenes", []), key=lambda item: item["scene_id"]):
        ...
        for raw in sorted(raw_scene.get("episodes", []), key=lambda item: (item["episode_index"], item["seed"])):
            episode = EpisodeSpec(scene_id=raw_scene["scene_id"], scene_label=raw_scene["scene_label"], scenario_id=raw["scenario_id"], episode_index=int(raw["episode_index"]), seed=int(raw["seed"]), start_pose=tuple(raw["start_pose"]), goal_pose=tuple(raw["goal_pose"]), episode_uid=raw.get("episode_uid", ""), navdp_seed=raw.get("navdp_seed"), source_episode_index=raw.get("source_episode_index"), selection_reason=raw.get("selection_reason", ""))
        ...
        scenes.append(SceneSpec(raw_scene["scene_id"], raw_scene["scene_label"], raw_scene["scene_path"], tuple(episodes), raw_scene.get("asset_hash", "")))
