...
class MockBackend:
    ...
    def run(self, run, episodes, writer):
        ...
        for episode in episodes:
            ...
            writer.submit_csv("planning_cycles", {**base, "episode_generation":0, "planning_cycle_uid":cycle_uid, "trigger_timestamp_s":.1, "raw_available":True, "candidate_count":2, "attempted_candidate_count":2 if run.variant != "raw" else 0, "selected_candidate_index":0 if published else "", "optimizer_success":False if failure else ("" if run.variant == "raw" else True), "cpp_validation_success":False if failure else ("" if run.variant == "raw" else True), "python_validation_success":False if failure else ("" if run.variant == "raw" else True), "stale":False, "published":published, "fallback_mode":fallback, "failure_reason":"MINCO_VALIDATION_FAILED" if failure else "", "navdp_ms":8.0, "minco_ms":0.0 if run.variant == "raw" else 6.0, "validation_ms":0.5, "planning_total_ms":12.0 if run.variant == "raw" else 18.0, "plan_age_when_applied_ms":5.0 if published else "", "data_source":"SIMULATED"})
            if published:
