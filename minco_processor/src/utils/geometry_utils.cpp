...
double geometry_utils::DistancePointEllipse(
  double e0, double e1, double y0, double y1, double & x0, double & x1)
{
  double distance;
...
double geometry_utils::DistancePointEllipsoid(
  double e0, double e1, double e2, double y0, double y1, double y2, double & x0, double & x1, double & x2)
{
...
template Eigen::Matrix<double, 3, 1> geometry_utils::quaternion_to_yrp<double>(
  const Eigen::Quaternion<double> & q_);

template Eigen::Matrix<float, 3, 1> geometry_utils::quaternion_to_yrp<float>(
  const Eigen::Quaternion<float> & q_);
  ...
  double & aT)
{
  ...
  // 如果需要与 tf 完全兼容的输出：
  if (rpy(2) < 0)
    rpy(2) += 2 * M_PI;  // 将 yaw 调整到 [0, 2pi) 范围
  Vec3f omega;
...
bool geometry_utils::pointInsidePolytope(const Vec3f & point, const PolyhedronH & polytope, double margin)
{
  Eigen::Vector4d pt_e;
  pt_e.head(3) = point;
  pt_e(3) = 1;
...
Vec3f geometry_utils::lineBoxIntersectPoint(
  const Vec3f & pt, const Vec3f & pos, const Vec3f & box_min, const Vec3f & box_max)
{
...
// 通过三点获得一个平面
void geometry_utils::FromPointsToPlane(const Eigen::Vector3d & p1,
...
void geometry_utils::getFovCheckPlane(const Eigen::Matrix3d R,
...
void geometry_utils::GetFovPlanes(const Eigen::Matrix3d R,
  const Eigen::Vector3d t,
  Eigen::MatrixX4d & fov_planes,
  std::vector<Eigen::Matrix3d> & fov_pts)
{
...
double geometry_utils::findInteriorDist(const Eigen::MatrixX4d & hPoly, Eigen::Vector3d & interior)
{
...
// Each row of hPoly is defined by h0, h1, h2, h3 as
// h0*x + h1*y + h2*z + h3 <= 0
bool geometry_utils::findInterior(const Eigen::MatrixX4d & hPoly, Eigen::Vector3d & interior)
{
  const int m = hPoly.rows();
...
bool geometry_utils::overlap(
  const Eigen::MatrixX4d & hPoly0, const Eigen::MatrixX4d & hPoly1, const double eps)
{
...
void geometry_utils::filterVs(const Eigen::Matrix3Xd & rV, const double & epsilon, Eigen::Matrix3Xd & fV)
{
...
// h0*x + h1*y + h2*z + h3 <= 0
// proposed epsilon is 1.0e-6
bool geometry_utils::enumerateVs(
  const Eigen::MatrixX4d & hPoly, Eigen::Matrix3Xd & vPoly, const double epsilon)
{
...
template <typename Scalar_t> Scalar_t geometry_utils::toRad(const Scalar_t & x)
{
  return x / 180.0 * M_PI;
}

template <typename Scalar_t> Scalar_t geometry_utils::toDeg(const Scalar_t & x)
{
  return x * 180.0 / M_PI;
}

template <typename Scalar_t> Eigen::Matrix<Scalar_t, 3, 3> geometry_utils::rotx(Scalar_t t)
{
...
template <typename Scalar_t> Eigen::Matrix<Scalar_t, 3, 3> geometry_utils::roty(Scalar_t t)
{
  Eigen::Matrix<Scalar_t, 3, 3> R;
  R(0, 0) = std::cos(t);
  R(0, 1) = 0.0;
...
template <typename Scalar_t> Eigen::Matrix<Scalar_t, 3, 3> geometry_utils::rotz(Scalar_t t)
{
  Eigen::Matrix<Scalar_t, 3, 3> R;
  R(0, 0) = std::cos(t);
  R(0, 1) = -std::sin(t);
...
Eigen::Matrix<typename Derived::Scalar, 3, 3> geometry_utils::ypr_to_R(
...
template <typename Derived>
Eigen::Matrix<typename Derived::Scalar, 3, 3> geometry_utils::vec_to_R(
  const Eigen::MatrixBase<Derived> & v1, const Eigen::MatrixBase<Derived> & v2)
{
  EIGEN_STATIC_ASSERT_FIXED_SIZE(Derived);
  EIGEN_STATIC_ASSERT(Derived::RowsAtCompileTime == 3, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
  EIGEN_STATIC_ASSERT(Derived::ColsAtCompileTime == 1, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
  ...
  Eigen::Quaternion<typename Derived::Scalar> q =
    Eigen::Quaternion<typename Derived::Scalar>::FromTwoVectors(v1, v2);
...
template <typename Derived>
Eigen::Quaternion<typename Derived::Scalar> geometry_utils::ypr_to_quaternion(
  const Eigen::DenseBase<Derived> & ypr)
{
  EIGEN_STATIC_ASSERT_FIXED_SIZE(Derived);
  EIGEN_STATIC_ASSERT(Derived::RowsAtCompileTime == 3, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
...
template <typename Scalar_t> Eigen::Quaternion<Scalar_t> geometry_utils::yaw_to_quaternion(Scalar_t yaw)
{
  return Eigen::Quaternion<Scalar_t>(rotz(yaw));
}

template <typename Scalar_t> Scalar_t geometry_utils::normalize_angle(Scalar_t a)
{
  int cnt = 0;
  while (true) {
    cnt++;
...
template <typename Scalar_t> Scalar_t geometry_utils::angle_add(Scalar_t a, Scalar_t b)
{
  Scalar_t c = a + b;
  c = normalize_angle(c);
...
template <typename Scalar_t> Scalar_t geometry_utils::yaw_add(Scalar_t a, Scalar_t b)
{
  return angle_add(a, b);
}

template <typename Derived>
Eigen::Matrix<typename Derived::Scalar, 3, 3> geometry_utils::get_skew_symmetric(
...
template <typename Derived>
Eigen::Matrix<typename Derived::Scalar, 3, 1> geometry_utils::from_skew_symmetric(
...
template Eigen::Matrix<double, 3, 1> geometry_utils::from_skew_symmetric<Eigen::Matrix3d>(
  const Eigen::DenseBase<Eigen::Matrix3d> & M);

template Eigen::Matrix<double, 3, 3> geometry_utils::get_skew_symmetric<Eigen::Vector3d>(
  const Eigen::DenseBase<Eigen::Vector3d> & v);
...
template Eigen::Matrix<double, 3, 3> geometry_utils::ypr_to_R(
  const Eigen::DenseBase<Eigen::Vector3d> & ypr);

template Eigen::Matrix<double, 3, 3> geometry_utils::vec_to_R(
  const Eigen::MatrixBase<Eigen::Vector3d> & v1, const Eigen::MatrixBase<Eigen::Vector3d> & v2);

template Eigen::Matrix<double, 3, 1> geometry_utils::R_to_ypr(const Eigen::DenseBase<Eigen::Matrix3d> & R);

template Eigen::Quaternion<double> geometry_utils::ypr_to_quaternion(
  const Eigen::DenseBase<Eigen::Vector3d> & ypr);

template Eigen::Matrix<double, 3, 1> geometry_utils::quaternion_to_ypr(
  const Eigen::Quaternion<double> & q_);
...
template Eigen::Matrix<float, 3, 1> geometry_utils::from_skew_symmetric<Eigen::Matrix3f>(
  const Eigen::DenseBase<Eigen::Matrix3f> & M);

template Eigen::Matrix<float, 3, 3> geometry_utils::get_skew_symmetric<Eigen::Vector3f>(
  const Eigen::DenseBase<Eigen::Vector3f> & v);
...
template Eigen::Matrix<float, 3, 3> geometry_utils::vec_to_R(
  const Eigen::MatrixBase<Eigen::Vector3f> & v1, const Eigen::MatrixBase<Eigen::Vector3f> & v2);

template Eigen::Matrix<float, 3, 1> geometry_utils::R_to_ypr(const Eigen::DenseBase<Eigen::Matrix3f> & R);

template Eigen::Quaternion<float> geometry_utils::ypr_to_quaternion(
  const Eigen::DenseBase<Eigen::Vector3f> & ypr);
